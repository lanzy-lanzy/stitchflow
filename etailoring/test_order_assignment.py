...
def test_order_assignment():
    ...
    print(f"Fabric quantity before assignment: {fabric.quantity}")
    print(f"Thread quantity before assignment: {thread.quantity}")
    print(f"Button quantity before assignment: {button.quantity}")
    
    # Assign order to tailor
    try:
        task = OrderManager.assign_order_to_tailor(order, tailor)
        print(f"Order #{order.id} successfully assigned to {tailor.user.first_name} {tailor.user.last_name}")
        print(f"Task #{task.id} created with status: {task.status}")
        print(f"Order status updated to: {order.status}")
        
        # Refresh from database to get updated quantities
        fabric.refresh_from_db()
        thread.refresh_from_db()
        button.refresh_from_db()
        
        print(f"Fabric quantity after assignment: {fabric.quantity}")
        print(f"Thread quantity after assignment: {thread.quantity}")
        print(f"Button quantity after assignment: {button.quantity}")
